...
import pandas as pd
import sys
from lm_benchmark.generate_util import *
...
def parseArgs(argv):
    ...
    parser.add_argument('--target_tokens',
                        default=['E', 'T', 'A', 'H', 'O', 'N', 'I', 'S', 'R', 'D', 'L', 'W', 'U', 'M', 'G', 'C',
                                 'F', 'Y', 'B', 'P', 'K', 'V', 'J', 'X', 'Q', 'Z', '|'], help='a list of target tokens')
    return parser.parse_args(argv)


def generate_beam(seed_text, beam_width, temp, length, token_dict, boundary_lst, target_tokens, task, generate_model,
                  gpu):
    '''
    Beam search
    ...
    while word_num < length:
        ...
        for output in outputs:  # different candidates
            ...
            for sequence in sequences:
                ...
                sentence_tokens = task.source_dictionary.encode_line(sequence, append_eos=False,
            ...
            sequences_inputs = torch.nn.utils.rnn.pad_sequence(input_sequences, batch_first=True,
        ...
        word_num = count_boundary(boundary_lst, top_candidate[
                                                2:])  # note here we count the generated words; remove the beginning intials
...
def generate_sampling(seed_text, sample_mode, temp, topk, length, token_dict, boundary_lst, target_tokens, task,
                      generate_model, gpu):
    """
        ...
        for temp in tqdm(temp_lst):
            ...
            while n < prompt.shape[0]:
                ...
                if prompt_mode == 'prompted':
                    prompt_text = word2char(
                        prompt['train'].tolist()[n])  # TO DO: add the designed prompts for the next step
                else:
                    prompt_text = '|'
                try:
                    # select different generation modes
                    if mode.split('_')[0] == 'sample':
                        generated = generate_sampling(prompt_text, mode, temp, decoding_para, length, token_dict,
                                                      boundary_lst, target_tokens, task, generate_model, gpu)

                    elif mode.split('_')[0] == 'beam':
                        generated = generate_beam(prompt_text, decoding_para, temp, length, token_dict, boundary_lst,
                                                  target_tokens, task, generate_model, gpu)
...
# Given dictionary
word_dict = {'|': 903350, 'E': 485501, 'T': 359694, 'A': 310633, 'O': 298737, 'N': 267067, 'I': 263289, 'H': 248238, 'S': 247973,
 'R': 224532, 'D': 168073, 'L': 156604, 'U': 108983, 'C': 95507, 'M': 93617, 'W': 91269, 'F': 86338, 'Y': 77772,
 'G': 76212, 'P': 65217, 'B': 56973, 'V': 35851, 'K': 28836, 'J': 6379, 'X': 6199, 'Q': 3705, 'Z': 2556, 'Â': 2144,
 '\x80': 1896, '\x94': 1736, '§': 426, '¢': 180, '°': 107, 'Ö': 82, 'Ä': 58, 'É': 38, '\x96': 33, 'Ñ': 32, '»': 28,
 '«': 27, '\x84': 25, 'Ë': 20, '£': 14, 'Ô': 13, 'Æ': 10, 'È': 10, 'Ü': 6, '\x9e': 5, 'Å': 4, 'Î': 4, '\x99': 3, '¦': 3,
 '©': 3, 'º': 1, '¼': 1, 'Ó': 1, 'Õ': 1, 'madeupword0000': 0, 'madeupword0001': 0, 'madeupword0002': 0}


word_lst = [item[0] for item in word_dict]
# output the matrix
...
vocabulary = ['<pad>', 'hello', 'world', '<unk>', '<eos>']

# Given tensor of indices
tensor_indices = torch.tensor([[1, 2, 3], [4, 0, 2]])  # Example tensor

# Convert tensor indices to string
strings = []
for indices in tensor_indices:
    tokens = [vocabulary[idx] for idx in indices]
    string = ''.join(tokens)
    strings.append(string)
# Join the strings
result = '\n'.join(strings)
with open('/scratch2/jliu/freq_bias_benchmark/model/100_cus/batch/' + str(i)+'.txt', 'w') as f:
    f.write(result)
print(result)
